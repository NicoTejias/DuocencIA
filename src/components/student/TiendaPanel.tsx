...
export default function TiendaPanel({ courses }: TiendaPanelProps) {
    ...
    const { data: rewards, isLoading } = useSupabaseQuery(
        () => RewardsAPI.getRewardsByCourse(selectedId),
        [selectedId]
    )
    ...
    const handleRedeem = async (rewardId: string) => {
        ...
        try {
            await RewardsAPI.redeemReward(rewardId, user?.id || '')
        ...
        <div className="max-w-6xl mx-auto py-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {isLoading ? (
                    ...
                    <>
                        {(rewards || []).map((r: any) => (
                            <div key={r.id} className="group bg-surface-light border border-white/5 rounded-[2rem] p-8 hover:bg-white/5 hover:border-primary/50 transition-all overflow-hidden relative">
                                ...
                                <div className="flex items-center justify-between pt-6 border-t border-white/5">
                                    ...
                                    >
                                        {processing === r.id ? <Loader2 className="w-4 h-4 animate-spin" /> : 'CANJEAR'}
                                    </button>
                                </div>
                            </div>
                        ))}
